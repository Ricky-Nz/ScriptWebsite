...
import { horCenter, fillHeight, fillHeightScroll } from './styles';
...
class HomeSection extends Component {
	render() {
		...
		const menuItems = this.props.versions.map((version, index) => (
			<MenuItem key={index} eventKey={version.download}>{version.name}</MenuItem>
		));
		const firstItem = this.props.versions.length > 0 ? this.props.versions[0].name : null;

		return (
			<Row style={fillHeight}>
				<Col xs={10} xsOffset={1} md={8} mdOffset={2} style={fillHeightScroll}>
					<br/><br/><br/><br/>
						...
						{firstItem ?
							<SplitButton id='version-button' style={center} bsStyle='primary'
								title={firstItem} onSelect={(e, key) => {
									...
									var dl = document.createElement('a');
									dl.setAttribute('href', this.props.versions[0].download);
									dl.click();
								}}>
								{menuItems}
							</SplitButton> : null
